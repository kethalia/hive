...
describe("CoderClient.login", () => {
  ...
  it("throws 'invalid credentials' on 401", async () => {
    ...
    await expect(CoderClient.login(BASE_URL, "bad@test.com", "wrong")).rejects.toThrow(
      "invalid credentials",
    );
  });
  ...
  it("throws on other HTTP errors", async () => {
    ...
    await expect(CoderClient.login(BASE_URL, "a@b.com", "pw")).rejects.toThrow("login failed: 500");
  });
