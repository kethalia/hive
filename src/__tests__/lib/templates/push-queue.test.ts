  ...
  describe("processor", () => {
    ...
    it("resolves per-user credentials and spawns coder with them", async () => {
      ...
      const jobPromise = capturedProcessor!({
        data: { templateName: "ai-dev-k8s", jobId: "job-1", userId: "user-abc" },
      });
    ...
    it("tees stdout and stderr to log file", async () => {
      const jobPromise = capturedProcessor!({
        data: { templateName: "ai-dev-k8s", jobId: "job-2", userId: "user-abc" },
      });
    ...
    it("writes [exit:0] sentinel on success", async () => {
      const jobPromise = capturedProcessor!({
        data: { templateName: "ai-dev-k8s", jobId: "job-3", userId: "user-abc" },
      });
    ...
    it("writes [exit:1] sentinel and rejects on non-zero exit", async () => {
      const jobPromise = capturedProcessor!({
        data: { templateName: "ai-dev-k8s", jobId: "job-4", userId: "user-abc" },
      });
    ...
    it("rejects on spawn error and writes error to log", async () => {
      const jobPromise = capturedProcessor!({
        data: { templateName: "ai-dev-k8s", jobId: "job-5", userId: "user-abc" },
      });
    ...
    it("writes failure log when userId has no token", async () => {
      ...
      await expect(
        capturedProcessor!({
          data: { templateName: "ai-dev-k8s", jobId: "job-6", userId: "bad-user" },
        }),
      ...
      const allLog = logChunks.join("");
      expect(allLog).toContain('[template-push] Starting push for "ai-dev-k8s"');
    ...
    it("writes failure log when coder binary is unavailable before spawn", async () => {
      ...
      await expect(
        capturedProcessor!({
          data: { templateName: "ai-dev-k8s", jobId: "job-7", userId: "user-abc" },
        }),
