...
export async function getCoderClientForUser(userId: string): Promise<CoderClient> {
  const db = getDb();
  ...
  if (!token) {
    ...
    if (!userExists) {
      ...
      throw new UserClientException(UserClientError.USER_NOT_FOUND, `User ${userId} not found`);
    }
