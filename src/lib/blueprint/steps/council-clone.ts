...
/**
 * Create the council-clone step.
 *
 * Clones the repository and checks out the PR branch inside the
 * council reviewer workspace so subsequent steps can inspect the code.
 *
 ...
 */
export function createCouncilCloneStep(): BlueprintStep {
  return {
    name: "council-clone",
    async execute(ctx) {
      ...
      if (result.exitCode !== 0) {
        ...
        console.log(`[blueprint] council-clone: ${msg} (task=${ctx.taskId})`);
