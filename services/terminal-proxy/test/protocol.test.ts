...
describe("buildPtyUrl", () => {
  ...
  it("constructs correct URL with all parameters including tmux command", () => {
    ...
    expect(parsed.searchParams.get("command")).toBe("tmux -L web new-session -A -s my-session \\; set status off \\; set mouse off");
  });
  ...
  it("includes tmux attach-or-create command with session name", () => {
    ...
    const parsed = new URL(url);
    expect(parsed.searchParams.get("command")).toBe("tmux -L web new-session -A -s dev-shell \\; set status off \\; set mouse off");
  });
