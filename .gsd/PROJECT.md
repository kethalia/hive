...
**M010 complete — all 4 slices delivered and verified.** Multi-user Coder authentication replacing static env vars with per-user, per-deployment credentials. PWA-capable with push notifications for token expiry warnings. 618 tests passing across 78 files. All 22 M010 requirements (R088-R109) validated.
...
- S03 (Token Lifecycle & Resilience): complete — Token auto-rotation at 75% lifetime via BullMQ hourly job with optimistic locking. Worker pre-flight expiry check refuses jobs with expired/key_mismatch tokens or <2h remaining. Network vs auth error classification (UnrecoverableError for auth, retry for network). tryDecrypt discriminated union for GCM key mismatch detection. TokenExpiryBanner in dashboard layout via server action. 51 tests. 5 requirements validated (R097, R098, R102, R105, R108).
- S04 (PWA & Push Notifications): complete — Web app manifest with display:standalone, service worker with push/notificationclick handlers, PushSubscription/VapidKeys Prisma models, VAPID key auto-generation and caching, push subscription server actions with auth, PushPermissionPrompt component in dashboard, sendPushToUser with stale subscription cleanup (410/404), token rotation integration at ≤24h threshold. 34 tests. 3 requirements validated (R103, R104, R109).
...
- **Authentication:** Multi-user Coder auth via direct login API. Per-user encrypted API keys in Postgres. Dynamic Coder URL per user. Browser sessions database-backed. Workers use submitting user's stored credentials. Token lifecycle: auto-rotation at 75% lifetime via BullMQ hourly job with optimistic locking, pre-flight expiry check refuses worker jobs with expired/<2h tokens, tryDecrypt detects encryption key changes per-user, in-app expiry banner in dashboard layout.
- **PWA & Push Notifications:** Web app manifest with display:standalone, minimal service worker (push + notificationclick only, no fetch/offline caching). VAPID keys auto-generated and persisted to DB. Push subscriptions per-user via PushSubscription model. Token expiry push notifications fire at ≤24h threshold during rotation. Stale subscription cleanup on 410/404 from push services. Notification click navigates to /login for re-authentication.
...
- [x] M010: Multi-User Coder Authentication — Replace static CODER_URL/CODER_SESSION_TOKEN with per-user auth via Coder's direct login API, encrypted token storage, PWA with push notifications (4 slices, 618 tests, 22 requirements validated)
