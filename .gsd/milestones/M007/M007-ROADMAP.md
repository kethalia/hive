...
| S02 | Terminal Integration & Session Management | medium | S01 | ⬜ | Terminal sessions listed under each workspace in sidebar. Clicking a session navigates to full-page terminal. All keystrokes captured by xterm. Session create/kill/switch from sidebar. Stale entry clicks trigger error + sidebar refresh. |
| S03 | Template Detail Page & Sidebar Polish | low | S01 | ⬜ | Clicking a template in sidebar opens detail page with info and push button. Sidebar mode toggle (pin/unpin) with localStorage persistence. Mobile responsive sidebar. Old workspaces page removed. |
