...
| S02 | Bidirectional Terminal via PTY WebSocket | high | S01 | ⬜ | User clicks 'new terminal' on a workspace, gets a full interactive shell — types commands, runs vim, closes the tab, reopens it, reattaches to the same tmux session with scrollback intact |
