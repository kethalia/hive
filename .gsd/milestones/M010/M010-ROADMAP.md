...
| S02 | Per-User Token Rewiring | medium | S01 | ⬜ | Submit a task — it runs end-to-end using submitting user's stored API key. No CODER_URL or CODER_SESSION_TOKEN in .env. Template push uses per-user token. |
| S03 | Token Lifecycle & Resilience | medium | S01 | ⬜ | Token nearing expiry auto-rotates. Worker refuses job with expired token (clear message). Encryption key change doesn't crash app. In-app expiry banner visible. |
| S04 | PWA & Push Notifications | low | S01, S03 | ⬜ | App installs as PWA. Push notification fires when token is 24h from expiry. Notification opens login page. Login page has Coder-like styling. |
