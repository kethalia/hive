...
const allowedArgs = new Set(["--allow-pending", "--self-test"]);
...
function isPendingOrBlocked(value) {
  return /\b(?:pending|blocked|todo|tbd|unknown|not captured|not observed|not available)\b/i.test(value);
}
...
const forbiddenPatterns = [
  ...
  },
  {
    ...
    regex: /(^|[\s`'"(=])\/(?!keepalive\/status\b|runtime-config\.js\b)(?:[A-Za-z0-9._-]+\/)+[A-Za-z0-9._-]+/m,
  },
  {
...
const finalChecks = [
  {
    ...
    ok: (fields) => /^(?:accepted|pass(?:ed)?|complete|validated)\b/i.test(fields.get("Run status") ?? ""),
  },
  {
    name: "Environment class is deployed or production-like",
    ok: (fields) => /\b(?:deployed|preview|production-like|prod-like|production)\b/i.test(fields.get("Environment class") ?? ""),
  },
  {
  ...
  },
  {
    ...
    ok: (fields) => /\b(?:idle|restart|restarted|network drop|disconnect|dropped|proxy|pod|rollout)\b/i.test(fields.get("Disconnect method") ?? ""),
  },
  {
    name: "Blast-radius approval state is safe or approved",
    ok: (fields) => /\b(?:approved|preview|non-shared|isolated|local|not required|n\/a)\b/i.test(fields.get("Blast-radius approval state") ?? ""),
  },
  {
    name: "Reload sentinel is preserved",
    ok: (fields) => /\b(?:preserved|unchanged|same|no reload|no page refresh)\b/i.test(fields.get("Reload-sentinel result") ?? ""),
  },
  {
    name: "Recovery UI observations show in-place recovery",
    ok: (fields) => /\b(?:recovered|reconnected|connected|recovery|healthy|cleared)\b/i.test(fields.get("Recovery UI observations") ?? ""),
  },
  {
    name: "Terminal or Git pane identity preservation is confirmed",
    ok: (fields) => /\b(?:preserved|same|unchanged|identity|mounted|pane)\b/i.test(fields.get("Terminal or Git pane identity preservation") ?? ""),
  },
  {
    name: "Post-recovery input usability is confirmed",
    ok: (fields) => /\b(?:accepted|usable|input|typed|received|interactive|confirmed|yes)\b/i.test(fields.get("Post-recovery input usability") ?? ""),
  },
  {
    name: "Resize/refit signal is present or Git-only N/A is stated",
    ok: (fields) => /\b(?:resize|refit|rows|cols|dimensions|n\/a|git-only|not terminal)\b/i.test(fields.get("Resize/refit signal") ?? ""),
  },
  {
  ...
  },
  {
    ...
    ok: (fields) => {
      ...
      return /\b(?:redacted|sanitized|category|count|healthy|active|connected|status)\b/i.test(value);
    },
  },
  {
    ...
    ok: (fields, markdown) =>
      ...
      ) && !/Connection failed after multiple attempts\. Refresh the page to try again\./i.test(markdown),
  },
  {
    name: "Final acceptance is recorded",
    ok: (fields) => /\b(?:accepted|pass(?:ed)?|validated|approved|yes)\b/i.test(fields.get("Final acceptance") ?? ""),
  },
];

function evaluate(markdown, { allowPending = false } = {}) {
  const results = [];
...
function buildFixture(overrides = {}) {
  const values = new Map([
    ...
    ["Sanitized keepalive/proxy status before", "sanitized status category/count summary with redacted workspace key"],
    ["Sanitized keepalive/proxy status after", "sanitized connected status category/count summary with redacted workspace key"],
...
function expectFixture(name, markdown, options, expectedPass) {
  ...
  if (passed !== expectedPass) {
    console.error(`FAIL self-test ${name}: expected ${expectedPass ? "pass" : "fail"}, got ${passed ? "pass" : "fail"}`);
    return false;
  }
...
function runSelfTest() {
  const fixtures = [
    ...
    ["pending run status fails final mode", buildFixture({ "Run status": "PENDING - not run" }), {}, false],
    ["pending run status passes pending mode", buildFixture({ "Run status": "PENDING - not run" }), { allowPending: true }, true],
    ["raw WebSocket URL fails pending mode", buildFixture({ "Recovery UI observations": "saw wss://terminal.example.invalid/ws reconnect" }), { allowPending: true }, false],
    ["full HTTP URL fails pending mode", buildFixture({ "Deployment or production-like preflight": "passed at https://hive.example.invalid" }), { allowPending: true }, false],
    ["absolute POSIX path fails pending mode", buildFixture({ "Coder workspace availability": "available at /home/coder/projects/kethalia/hive" }), { allowPending: true }, false],
    ["absolute Windows path fails pending mode", buildFixture({ "Coder workspace availability": "available at C:\\\\Users\\\\coder\\\\repo" }), { allowPending: true }, false],
    ["UUID-like workspace ID fails pending mode", buildFixture({ "Coder workspace availability": "workspace 123e4567-e89b-12d3-a456-426614174000 available" }), { allowPending: true }, false],
    ["token key fails pending mode", buildFixture({ "Recovery UI observations": "token=abc123secret" }), { allowPending: true }, false],
    ["clone proof key fails pending mode", buildFixture({ "Terminal or Git pane identity preservation": "cloneProof=abc123" }), { allowPending: true }, false],
    ["terminal transcript marker fails pending mode", buildFixture({ "Post-recovery input usability": "terminal transcript shows prompt$ pwd" }), { allowPending: true }, false],
    ["raw session name key fails pending mode", buildFixture({ "Terminal or Git pane identity preservation": "sessionName=git-clone-raw" }), { allowPending: true }, false],
    ["raw proxy error fails pending mode", buildFixture({ "Recovery UI observations": "proxy error: ECONNRESET" }), { allowPending: true }, false],
    [
      "unredacted keepalive workspace key fails pending mode",
      buildFixture({
        "Sanitized keepalive/proxy status after": '/keepalive/status {"workspace-raw-key":{"status":"active"}}',
      }),
      { allowPending: true },
      false,
    ],
...
const results = evaluate(evidence, { allowPending });
const passed = summarize(results, allowPending ? "M006 S05 pending evidence contract" : "M006 S05 final evidence");
