...
- Startup probes are supported but render only when explicitly configured.
...
- Startup probes are empty by default. Add service-specific startup probes in production overlays only if cold starts need more time than readiness and liveness allow.
