...
test.describe("authenticated Hive workflows", () => {
  ...
  test("covers workspace launch and global navigation", async ({ page }, testInfo) => {
    ...
    await page.keyboard.press(`${navigationModifier}+Shift+3`);
    await expect(page).toHaveURL(/\/terminal\/status$/);
    ...
    await expect(page.getByText("Launch workspace", { exact: true }).last()).toBeVisible();
    await page.keyboard.press("Escape");
    await expect(page.getByRole("dialog")).toBeHidden();

    await page.goto(new URL("/workspaces?launch=1", appUrl).toString());
    await expect(page.getByRole("heading", { name: "Launch workspace" })).toBeVisible();
    await waitForDashboardReady(page);
    await expect(page.getByLabel("Workspace profile")).toBeVisible();
    await expect(page.getByTestId("selected-workspace-profile")).toBeVisible();
    await capture(page, testInfo, "launch-workspace");
